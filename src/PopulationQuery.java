...
public class PopulationQuery {
	...
	public static Pair<Integer, Float> singleInteraction(int w, int s, int e,
			int n, Version processor) {
		return processor.singleInteraction(w, s, e, n);
	}
	
	public static Version preprocess(String filename, int columns, int rows,
			int versionNum) {
		
		CensusData parsedData = parse(filename);
		if (versionNum == 1) {
			return new Version1(parsedData, columns, rows);
		} else if (versionNum == 2) {
			return new Version2(parsedData, columns, rows);
		}
	...
	public static void main(String[] args) {
		...
		Version processor = preprocess(args[0], Integer.parseInt(args[1]), Integer.parseInt(args[2]),
		...
		while (true) {
			...
			// And finally, execute
			// 0 based indexing
			Pair<Integer, Float> results = singleInteraction(west - 1, south - 1, east - 1, north - 1, processor);
